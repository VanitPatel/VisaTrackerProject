...
async function setupDb(db) {
  ...
  // ── Profile column migrations (safe — ignored if column already exists) ──
  const profileCols = [
  ...
  for (const col of profileCols) {
    try { await db.exec(col + ';'); } catch(_) { /* column already exists */ }
  }
}
...
Use **bold** for all deadlines, compliance scores, and critical findings. Write in formal audit language.`;
    ...
    // ── GET /account/profile ──────────────────────────────
    // Returns the current user's saved profile fields
    ...
    // Body: { full_name, email, organization, phone, timezone, bio }
    // Saves profile fields — no password required
    if (path === '/account/profile' && request.method === 'POST') {
      if (!user) return err('Not authenticated', 401);
      const { full_name, email, organization, phone, timezone, bio } = await request.json();
      ...
      await db.prepare(`
        ...
        WHERE id = ?
      `).bind(
        (full_name    || '').trim().slice(0, 100),
        (email        || '').trim().slice(0, 200),
        (organization || '').trim().slice(0, 150),
        (phone        || '').trim().slice(0, 30),
        (timezone     || 'America/New_York').slice(0, 60),
